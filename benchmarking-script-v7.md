...
That's single-main n8n. One Docker container handling webhooks, executions, database writes — everything running in the same process, competing for the same resources.

And here's the thing about a food truck kitchen. It usually comes with a tiny pantry. A small cabinet for storage. Even if the chef has energy to spare, if that pantry can't supply ingredients fast enough, everything slows down.
...
There's one limitation: your team shares a single workspace for building and editing workflows — one UI instance. You can scale to serve as many customers as you want, even on community edition, but if that workspace goes down, your developers can't work.
...
That's multi-main mode, unlocked with an enterprise license. You can deploy multiple UI instances — multiple test kitchens for your team to build workflows. All of them write to the same Redis queue, all of them share the same database. High availability. True redundancy.
...
Single webhook test. Three virtual users: 15 requests per second. Ten users: still 15. Thirty, fifty, a hundred users — stuck at 15 RPS. Response times are climbing, but throughput is flatlined.
...
Most people see these symptoms and think: time for a bigger database server. Or time to add workers. Upgrading that c5.large to a c5.xlarge would cost you an extra ~$50/month.
...
PGTune. It's been around for years. It's the industry standard for PostgreSQL configuration.
...
If you're using Docker Compose — and most self-hosted deployments are — you mount a custom postgresql.conf file and tell Postgres to use it. Here's all you need:
...
This is the pure win. No architecture changes, no new servers, no bigger instance. Just telling PostgreSQL to use what it already had.
...
That's the scaling problem you want to have. The pantry can keep up, but there's not enough hands to plate the food. Time to upgrade to a restaurant — move to queue mode, spin up dedicated webhook workers, execution workers, let Redis distribute the tickets.
...
You've hit an architecture wall. You've outgrown your current kitchen. If you're on queue mode, it might be time for multi-main. If you're handling lots of binary data — large file uploads and processing — you might need external storage solutions. Binary workloads aren't pantry problems. They're hitting a different ceiling: memory, disk I/O, the architecture itself. It's like trying to cater a wedding out of a food truck. Even with a fully built-out pantry, you need a different kind of kitchen.

In our benchmarks, binary data stayed flat at ~3 RPS before and after tuning — proof that not every bottleneck lives in the database.
...
We started with a mystery: capacity sitting unused, throughput stuck at 15 requests per second. The fix wasn't a bigger kitchen. It wasn't more workers. It was building out the pantry we already had.

PGTune. Free. Zero infrastructure changes. Fifteen requests per second to 93 — six times the throughput on the same hardware. What would upgrading the instance have cost you? Fifty bucks a month, minimum. PGTune costs zero.

From there, when you need more hands, you move to a restaurant — queue mode. When you need redundancy and multiple workspaces for your team, you go franchise — multi-main. Each step builds on the last, and tuning the pantry first makes every step better.

PGTune link is in the description — bookmark it. CloudFormation templates that automate the entire deployment? That's the next video.

That mystery from the cold open — why was there capacity sitting unused while throughput flatlined? Now you know. The capacity was always there. You just had to let PostgreSQL use it.
